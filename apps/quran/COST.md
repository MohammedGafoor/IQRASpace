...
| CDN/DNS (Cloudflare) | DNS hosting for `iqraspace.org` — confirmed 2026-08-31 via `vercel domains inspect` (registrar: "Third Party", nameservers already on Cloudflare) | Two A records (`iqraspace.org`, `www.iqraspace.org`), DNS-only mode (not proxied — `DEPLOYMENT.md`'s DNS section) | Free | $0 | Already in use; not a new decision. Kept in "DNS only" mode deliberately so Vercel's own edge/TLS handles all real traffic — no CDN-in-front-of-CDN conflict. |
